...
Limits Play enforces, and `store-kit validate` checks: short description ≤ 80
characters, full description ≤ 4000, release notes ≤ 500.

**Every claim below was checked against the code, not against the older docs.**
The five recognised speech languages (Mandarin, Cantonese, English, Japanese,
Korean) come from `TranscriptionLanguage`; the five *interface* languages come
from `lib/l10n/app_*.arb` and are a different set — Cantonese has no separate
written locale, so the UI ships in English, both Chinese scripts, Japanese and
Korean. The model size is `ModelDownloadService.expectedBytes` (239,233,841
bytes ≈ 228 MB). The "one network request" claim is literal: `http` appears in
exactly one file, `model_download_service.dart`.
...
```
Voice recorder with offline speech-to-text. Your audio never leaves the phone.
```
...
• Keeps recording when you leave the app or lock the screen, with a notification you can stop it from.
...
• The speech model is downloaded once (about 228 MB) and then kept on the device. After that, transcription works in flight mode.
...
PRIVACY
Your recordings never leave the phone. The app makes exactly one kind of network request in its life: downloading the speech model. There are no ads, no analytics, no account and no tracking. Recordings are stored in the app's own folder and are yours to share or delete.
```
...
• Record in the background, pause and resume, and skip silence.
• Offline speech-to-text in Mandarin, Cantonese, English, Japanese and Korean, detected automatically per phrase.
• Live text while recording — hold the subtitles button.
• Search by what was said and play from that moment.
• History with a month calendar.
• Light and dark themes; interface in five languages.
```
...
```
オフラインで文字起こしできるボイスレコーダー。音声は端末の外に出ません。
```
...
• アプリを離れても画面をロックしても録音は続きます。通知からそのまま停止できます。
...
プライバシー
録音が端末から出ることはありません。このアプリが行う通信は、音声モデルのダウンロードだけです。広告も、解析も、アカウントも、トラッキングもありません。
```
...
• バックグラウンド録音、一時停止と再開、無音スキップ。
• オフラインの文字起こし。中国語（標準語）・広東語・英語・日本語・韓国語をフレーズごとに自動判別します。
• 録音中のライブ字幕（字幕ボタンを長押し）。
• 話した言葉で検索し、その時点から再生。
• カレンダー付きの履歴。
• ライト／ダークテーマ、表示言語は5言語。
...
```
오프라인 받아쓰기가 되는 음성 녹음기. 녹음은 기기 밖으로 나가지 않습니다.
```
...
• 앱을 벗어나거나 화면을 잠가도 녹음은 계속되고, 알림에서 바로 멈출 수 있습니다.
...
개인정보
녹음은 기기 밖으로 나가지 않습니다. 이 앱이 하는 통신은 음성 모델을 내려받는 것 하나뿐입니다. 광고도, 분석도, 계정도, 추적도 없습니다.
```
...
• 백그라운드 녹음, 일시정지와 재개, 무음 건너뛰기.
• 오프라인 받아쓰기. 중국어(표준어), 광둥어, 영어, 일본어, 한국어를 문장 단위로 자동 인식합니다.
• 녹음 중 실시간 자막 — 자막 버튼을 누르고 있으면 됩니다.
• 말한 내용으로 검색하고 그 지점부터 재생.
• 달력이 있는 기록 화면.
• 라이트·다크 테마, 5개 언어 지원.
```
...
隐私
录音不会离开这台手机。这个应用一生中只发起一类网络请求：下载语音模型。没有广告，没有统计，没有账号，没有追踪。
```
...
• 后台录音，暂停与继续，跳过静音。
• 离线转文字，支持普通话、粤语、英语、日语、韩语，按句自动识别语种。
• 录音时按住字幕按钮，实时出字。
• 按说过的话搜索，并从那一刻开始播放。
• 带日历的历史记录。
• 浅色与深色主题，界面支持五种语言。
...
隱私
錄音不會離開這支手機。這個應用程式一生中只發出一種網路請求：下載語音模型。沒有廣告，沒有統計，沒有帳號，沒有追蹤。
```
...
• 背景錄音，暫停與繼續，略過靜音。
• 離線轉文字，支援國語、粵語、英語、日語、韓語，逐句自動判斷語言。
• 錄音時按住字幕按鈕，即時出字。
• 用說過的話搜尋，並從那一刻開始播放。
• 附月曆的歷史紀錄。
• 淺色與深色主題，介面支援五種語言。
